...
public class CpaAddInternalOperation extends CpaAbstractOperation {
	...
	public CpaAddInternalOperation(Registry registry, SpaObjectRegistry spaRegistry){
	...
	public List<SpaControl> prepare() throws RepositoryException {
		try {
			...
			KeyCorrectionControl keyCorrectionControl = new KeyCorrectionControl(persistanceDetailObject,detailMapper.getPrimaryKeyFromRepositoryObject(detailObject),OperationLevel.OPERATION,registryName);
			keyCorrectionControl.setMaster(persistanceMasterObject);
			keyCorrectionControl.setMasterMapper(masterMapper);
			...
			SpaControl notiificatioinControl = this.notificationOperation(detailMapper, persistanceDetailObject, NotificationType.INSERT);
			
			List<SpaControl> list= new ArrayList<>();
			list.add(masterControl);
			if ( !isUpdateStorageCommand()) {
				list.add(keyCorrectionControl);
			}
