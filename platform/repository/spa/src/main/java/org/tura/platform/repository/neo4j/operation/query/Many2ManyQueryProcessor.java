...
package org.tura.platform.repository.neo4j.operation.query;
...
import org.tura.platform.repository.neo4j.operation.Neo4jSearchService;
...
public class Many2ManyQueryProcessor<T> extends ExternalQueryProcessor<T>{
	...
	public SearchResult<T> process(List<SearchCriteria> searchCriteria, List<OrderCriteria> orderCriteria,
		...
		searchCriteria =  getSearchService().adoptSearchCriterias(searchCriteria, many2manyHelperClass);
		
		 List<?> ls = this.findObjectsQuery(searchCriteria, new ArrayList<OrderCriteria>(), 0, 100, many2manyHelperClass);
		 
		 if ( ls ==null ||  ls.size() == 0) {
			 return new SearchResult<T>( new ArrayList<T>(), 0);
		 }
		 Object many2manyObject = ls.get(0);
		 loadRelations((T) many2manyObject);
