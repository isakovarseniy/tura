...
public class SharedClientScopeContext implements Context {
	...
	private transient Logger logger = Logger.getLogger(SharedClientScopeContext.class.getName());

	private static final ThreadLocal<AtomicReference<BeanStorage>> cache = ThreadLocal
			.withInitial(AtomicReference::new);

	private static final ThreadLocal<AtomicReference<ScopeStorage>> STORAGE_THREAD_LOCAL = ThreadLocal
			.withInitial(AtomicReference::new);
	...
	private BeanStorage loadFromStorage(Scope scopeId) {
		try {
			if (isScopePresentInStorage(scopeId)) {
				ObjectStorage objectStorage = (ObjectStorage)(this.getStorage().load(scopeId));
				objectStorage.getLockerSave().restore();
				return objectStorage.getBeanStorage();
			}
		} catch (Exception e) {
			logger.info(ExceptionUtils.getStackTrace(e));
		}
		...
		return new SharedBeanStorage(100);
	}

	private void writeToStorage(Scope scopeId, BeanStorage instanceMap, LockerSave lockerSave) {
		try {
			if (this.getStorage() != null) {
				ObjectStorage objectStorage = new ObjectStorage(instanceMap, lockerSave);
				this.getStorage().write(scopeId, objectStorage);
			}
		} catch (Exception e) {
			logger.info(ExceptionUtils.getStackTrace(e));
		}
	...
	public void passivate(Scope scopeId) {
		if (cache.get().get() != null) {
			LockerSave lockerSave = new LockerSave();
			lockerSave.save();
			writeToStorage(scopeId, cache.get().get(),lockerSave);
	...
	public Class<? extends Annotation> getScope() {
		return SharedClientScoped.class;
	}
