...
public class CdiServerRegistry extends Registry {
	...
	public void init() {
		...
		try {
			...
			InitJPARepository initJpa = new InitJPARepository(this, spaRegistry);
			initJpa.initClassMapping();
			initJpa.initCommandProducer(new CdiEntityManagerProvider(), queryProcessorRegistry);
			...
			spaRegistry.getRegistry(InitSPARepository.SPA_REPOSITORY_NAME).addInstantiator(
					new TestServiceInstantiator(this, spaRegistry, InitSPARepository.SPA_REPOSITORY_NAME));

			spaRegistry.getRegistry(InitSPARepository.SPA_REPOSITORY_NAME).addCRUDProvider(org.tura.jpa.test.U1.class,
					CRUDService.class);
			spaRegistry.getRegistry(InitSPARepository.SPA_REPOSITORY_NAME)
					.addSearchProvider(org.tura.jpa.test.U1.class, SearchService.class);

			spaRegistry.getRegistry(InitSPARepository.SPA_REPOSITORY_NAME).addCRUDProvider(org.tura.jpa.test.Rref.class,
					CRUDService.class);
			spaRegistry.getRegistry(InitSPARepository.SPA_REPOSITORY_NAME)
					.addSearchProvider(org.tura.jpa.test.Rref.class, SearchService.class);
