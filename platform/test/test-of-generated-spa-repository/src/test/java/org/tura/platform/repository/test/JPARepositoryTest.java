...
public class JPARepositoryTest extends JPARepositoryAbstractTest{
	...
	public void t0002_insertIndependentObject() {
		try {
			...
			List<org.tura.spa.test.Location> listLocatioin = query.getResultList();
			org.tura.spa.test.Location loc = listLocatioin.iterator().next();
			assertEquals(1, loc.getCustomer().size());

			query = em.createQuery("from Customer");
			List<org.tura.spa.test.Customer> listCustomer = query.getResultList();
			org.tura.spa.test.Customer ctr = listCustomer.iterator().next();
