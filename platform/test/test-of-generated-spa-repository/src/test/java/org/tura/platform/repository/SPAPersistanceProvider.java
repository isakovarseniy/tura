...
public class SPAPersistanceProvider implements PersistenceProvider {
	...
	public void connect(Object srcPk, String srcMethod, Class<?> srcType, Object trgPk, String trgMethod,
		...
		if ("One2One".equals(relation)) {
			Reflection.callTyped(srcObject, "set" + srcMethod, trgObject.getClass(), trgObject);
			Reflection.callTyped(trgObject, "set" + trgMethod, srcObject.getClass(), srcObject);
			return;
	...
	public <T> T find(Class<T> clazz, Object pk) throws Exception {
		...
		Constructor<?> c =   clazz.getConstructor(PersistenceProvider.class);
		Object object = c.newInstance(this);
	...
	private Object findInternal(Class<?> clazz, Object pk) throws Exception {
		...
		String pkObjectName = pk.getClass().getName();
		Object newpk = pk;
		if (pkObjectName.contains(".spa.")) {
			String pkJpatName = pkObjectName.replace(".spa.", ".jpa.");
			Class<?> pkJpaClass = Class.forName(pkJpatName);
			newpk = pkJpaClass.newInstance();
			BeanUtils.copyProperties(newpk, pk);
		}

		return em.find(jpaClass, newpk);
	...
	public long findNumberOfRowsQuery(List<SearchCriteria> searchCriteria, List<OrderCriteria> orderCriteria,
		...
		preprocessor.process(searchCriteria, orderCriteria, Class.forName(className));

		SelectQuery dslQuery = preprocessor.getQuery();
		dslQuery.getColumns()[0] = c("count(*)");
		dslQuery.getOrderBy().clear();

		Query query = em.createNativeQuery(dslQuery.toSql());
	...
	public List<?> findObjectsQuery(List<SearchCriteria> searchCriteria, List<OrderCriteria> orderCriteria,
		...
		SelectQuery dslQuery = preprocessor.getQuery();
		Query query = em.createNativeQuery(dslQuery.toSql(), clazz);
		...
		for (Object obj : list) {
			...
			Constructor<?> c =   cl.getConstructor(PersistenceProvider.class);
