...
import org.tura.example.ui.hrmanager.tree2tree.datacontrol.CityDC;
...
public class CityDCStructureTest {
	...
	public void checkCityDCStructure(){
		assertEquals(20, CityDC.class.getDeclaredMethods().length);
		...
		assertTrue(findMethod( CityDC.class,"setCreateCommand" ,new Class[]{CreateCommandBase.class}));
		m = getMethod( CityDC.class,"setCreateCommand" ,new Class[]{CreateCommandBase.class});
		checkSelector(m,"hrmanager.tree2tree");
		checkCreateTrigger(m,"city");
		ArrayList<CallParameter> prms = new ArrayList<>();
		prms.add(new CallParameter("objectType", "org.elsoft.platform.hr.objects.CityDAO", TuraObject.class));
		checkCreateTriggerParameters(m,prms);
		
		
		assertTrue(findMethod( CityDC.class,"setInsertCommand" ,new Class[]{InsertCommandBase.class}));
		m = getMethod( CityDC.class,"setInsertCommand" ,new Class[]{InsertCommandBase.class});
		checkSelector(m,"hrmanager.tree2tree");
		checkInsertTrigger(m,"city");
		prms = new ArrayList<>();
		...
		assertTrue(findMethod( CityDC.class,"setUpdateCommand" ,new Class[]{UpdateCommandBase.class}));
		m = getMethod( CityDC.class,"setUpdateCommand" ,new Class[]{UpdateCommandBase.class});
		checkSelector(m,"hrmanager.tree2tree");
		checkUpdateTrigger(m,"city");
		prms = new ArrayList<>();
		prms.add(new CallParameter("object", "#{beanFactoryHrManagerTree2tree.treeRootCompany.controls['*******'].currentObject}", TuraObject.class));
		checkUpdateTriggerParameters(m,prms);

		
		assertTrue(findMethod( CityDC.class,"setDeleteCommand" ,new Class[]{DeleteCommandBase.class}));
		m = getMethod( CityDC.class,"setDeleteCommand" ,new Class[]{DeleteCommandBase.class});
		checkSelector(m,"hrmanager.tree2tree");
		checkDeleteTrigger(m,"city");
		prms = new ArrayList<>();
		prms.add(new CallParameter("object", "#{beanFactoryHrManagerTree2tree.treeRootCompany.controls['*******'].currentObject}", TuraObject.class));
		checkDeleteTriggerParameters(m,prms);

		
		assertTrue(findMethod( CityDC.class,"setSearchCommand" ,new Class[]{SearchCommandBase.class}));
		m = getMethod( CityDC.class,"setSearchCommand" ,new Class[]{SearchCommandBase.class});
		checkSelector(m,"hrmanager.tree2tree");
