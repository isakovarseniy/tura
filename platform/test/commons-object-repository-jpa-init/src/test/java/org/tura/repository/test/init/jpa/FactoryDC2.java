...
public class FactoryDC2  {
	...
	public DataControl initEmployees(String elPrefix,StorageProvider storageProvider) throws Exception {
		...
		pager.setRepositoryProvider(storageProvider.getCpaRepositoryProvider());
		...
		createOrderBy(employeesDS, "objId",Order.ASC.name());
		
		return employeesDS;
	}

	@SuppressWarnings({ "rawtypes" })
	public DataControl initDepartments(String elPrefix,StorageProvider storageProvider) throws Exception {
		...
		createOrderBy(departmentsDS, "objId",Order.ASC.name());

		return departmentsDS;
	}

	void createOrderBy(DataControl<?> control, String field, String orderBy) {
		
		OrderCriteria or = new OrderCriteria( field, orderBy);
		List<OrderCriteria> orders = new ArrayList<OrderCriteria>() ;
		orders.add(or);
		control.setDefaultOrderCriteria(orders);
	}
