...
public class DepartmentsDC extends DataControl<DepartmentsDAO> {
	...
	public void setDefaultQuery(
			@Query(base = @Base(clazz = DepartmentsDAO.class), search = @DefaultSearchCriterias(criterias = { }), orders = @DefaultOrderBys(orders = { @DefaultOrderBy(field = "objId", order = SelectQuery.Order.ASC) })) SelectQuery selectQuery) {
	...
	public IDataControl getEmployeesdc() {
		IDataControl dc = employeesdcproducers.get();
		getElResolver().setValue("employees", dc);
		return dc;
	}
