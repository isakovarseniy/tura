...
import org.elsoft.platform.metamodel.processor.uicontainer.model.TabPage;

public class CreateUITabPage {
	public static String CREATEUITABPAGE = "org.elsoft.platform.metamodel.objects.command.form.ui.CreateUITabPageDAO";

	public static int load(RepositoryFactory rf,
			HashMap<String, Object> context, CommandDAO cmd) {
		CreateUITabPageDAO command = (CreateUITabPageDAO) cmd;

		ChildrenOwner cw = (ChildrenOwner) context.get(command.getParentUUID());
		cw.addChild(new TabPage(command, context));
	...
	public static void save(CommandHandler ch, String parentUUID,
		...
		ch.getMode().getElResolver()
				.setValue(SessionContext.COMMAND_TYPE, CREATEUITABPAGE);
		DataControl<CommandDAO> dc = ch.getMode().getControl();
		CreateUITabPageDAO obj = (CreateUITabPageDAO) dc.createObject();
		obj.setParentUUID(parentUUID);
		obj.setCss(tab.getCss());
		obj.setCssClass(tab.getCssClass());
		obj.setCommandExecutor(CreateUITabPage.class.getName());
		obj.setUUID(tab.getUuid());
