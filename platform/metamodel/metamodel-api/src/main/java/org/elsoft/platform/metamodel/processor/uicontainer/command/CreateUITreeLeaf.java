...
import org.elsoft.platform.metamodel.processor.uicontainer.model.TreeLeaf;

public class CreateUITreeLeaf {

	public static String CREATEUITREELEAF = "org.elsoft.platform.metamodel.objects.command.form.ui.CreateUITreeLeafDAO";

	public static int load(RepositoryFactory rf,
			HashMap<String, Object> context, CommandDAO cmd) {
		CreateUITreeLeafDAO command = (CreateUITreeLeafDAO) cmd;
		ChildrenOwner cw = (ChildrenOwner) context.get(command.getParentUUID());
		cw.addChild(new TreeLeaf(command, context));
	...
	public static void save(CommandHandler ch, String parentUUID,
		...
		ch.getMode().getElResolver()
				.setValue(SessionContext.COMMAND_TYPE, CREATEUITREELEAF);
		DataControl<CommandDAO> dc = ch.getMode().getControl();
		CreateUITreeLeafDAO obj = (CreateUITreeLeafDAO) dc.createObject();
		obj.setParentUUID(parentUUID);
		obj.setCss(treeLeaf.getCss());
		obj.setCssClass(treeLeaf.getCssClass());
		obj.setCommandExecutor(CreateUITreeLeaf.class.getName());
		obj.setUUID(treeLeaf.getUuid());
	}
}
