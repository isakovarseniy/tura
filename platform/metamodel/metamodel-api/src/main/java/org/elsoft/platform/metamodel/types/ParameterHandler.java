...
@Base(clazz = ParameterDAO.class)
@VersionField(field = "version")
@IdField(field = "objId")
public class ParameterHandler extends Handler<ParameterHandler, ParameterDAO> {
	...
	@CreateTrigger(objectAction = "create", parameters = @Variables(value = { @Variable(value = "org.elsoft.platform.metamodel.objects.type.ParameterDAO", type = String.class) }), object = "generalService")
	...
	@SearchTrigger(objectAction = "find", parameters = @Variables(value = {
			...
			@Variable(type = java.lang.Integer.class),
			@Variable(value = "org.elsoft.platform.metamodel.objects.type.ParameterDAO", type = String.class) }), object = "generalService")
	private Method search;
	@GetMode
	private Mode<ParameterDAO> mode;
	@Connection(links = { @Link(field1 = "parameterType", field2 = "objId") }, connectedObject = TypeDefinitionHandler.class, connectionName = "Parameter2Type")
	...
	public Mode<ParameterDAO> getMode() {
		return mode;
	}

	public ParameterDAO addParameter(String prName) {
		ParameterDAO obj = getMode().getControl().createObject();
		obj.setParameterName(prName);
		obj.setOverwritten(true);
		return obj;
	}
