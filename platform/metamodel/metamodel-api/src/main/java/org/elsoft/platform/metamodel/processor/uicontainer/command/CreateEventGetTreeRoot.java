...
public class CreateEventGetTreeRoot {
	public static String CREATEEVENTGETTREEROOT = "org.elsoft.platform.metamodel.objects.command.EventDAO";
	...
	public static int load(RepositoryFactory rf,
		...
		command.setEventType(MetamodelTriggerEventsType.CreateEventGetTreeRoot.name());

		Tree element = (Tree) context.get(command.getParentUUID());

		element.setGetTreeRootEvent(command, context);
	...
	public static void save(CommandHandler ch, String parentUUID,
		...
		ch.getMode().getElResolver()
				.setValue(SessionContext.COMMAND_TYPE, CREATEEVENTGETTREEROOT);
		...
		obj.setEventType(MetamodelTriggerEventsType.CreateEventGetTreeRoot.name());
		obj.setCommandExecutor(CreateEventGetTreeRoot.class.getName());
	}
