package com.turasolutions.sirius.diagram.producer.tura.config.items.permission;
...
public class GroupConfigurator implements NodeConfigurator {
	public static tNode create() {
		return new tNode(new GroupConfigurator());
	}
	...
	public NodeStyleDescription getStyle() {
		...
		style.setWorkspacePath("/org.tura.metamodel.sirius.diagram.designer/images/group.png");
	...
	public static List<ToolEntry> getTools() {
		...
		ToolEntry tool = getGroupNodeCreationDescription();
		list.add(tool);
	...
	private static ToolEntry getGroupNodeCreationDescription() {
		...
		tool.setIconPath("/org.tura.metamodel.sirius.diagram.designer/icons/group.png");
		...
		CreateInstance c2 = ToolHelper.createInstance("permission.Group", "instance", "groups");
		c1.getSubModelOperations().add(c2);
		c2.getSubModelOperations().add(ToolHelper.createSet("uid", "service:generateUID"));

		ObjectWrapper wrapper = (ObjectWrapper) tRoot.context.get("Group" + tNode.class.getName());
	...
	private static ToolEntry getDirectEditLabel() {
		...
		tool.setName("Groups edit label");
