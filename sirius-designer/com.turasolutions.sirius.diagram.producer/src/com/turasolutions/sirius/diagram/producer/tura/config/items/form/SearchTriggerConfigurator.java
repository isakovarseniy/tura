...
public class SearchTriggerConfigurator implements NodeConfigurator {
	public static tNode create() {
		return new tNode(new SearchTriggerConfigurator());
	}
	...
	public static List<ToolEntry> getTools() {
		...
		ToolEntry tool = getSearchTriggerNodeCreationDescription();
		list.add(tool);
	...
	private static ToolEntry getSearchTriggerNodeCreationDescription() {
		NodeCreationDescription tool = ToolFactory.eINSTANCE.createNodeCreationDescription();
		tool.setName("SearchTrigger");
		...
		CreateInstance c2 = ToolHelper.createInstance("form.SearchTrigger", "instance", "search");
		c1.getSubModelOperations().add(c2);
		c2.getSubModelOperations().add(ToolHelper.createSet("uid", "service:generateUID"));

		ObjectWrapper wrapper = (ObjectWrapper) tRoot.context.get("SearchTrigger" + tNode.class.getName());
	...
	private static ToolEntry getDirectEditLabel() {
		...
		tool.setName("SearchTrigger edit label");
