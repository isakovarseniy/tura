...
public class PREQueryTriggerConfigurator implements NodeConfigurator {
	public static tNode create() {
		return new tNode(new PREQueryTriggerConfigurator());
	}
	...
	public String getCandidates() {
		return "feature:preQueryTrigger";
	}
	...
	public static List<ToolEntry> getTools() {
		...
		ToolEntry tool = getPREQueryTriggerNodeCreationDescription();
		list.add(tool);
	...
	private static ToolEntry getPREQueryTriggerNodeCreationDescription() {
		NodeCreationDescription tool = ToolFactory.eINSTANCE.createNodeCreationDescription();
		tool.setName("PREQueryTrigger");
		...
		CreateInstance c2 = ToolHelper.createInstance("form.PREQueryTrigger", "instance", "preQueryTrigger");
		c1.getSubModelOperations().add(c2);
		c2.getSubModelOperations().add(ToolHelper.createSet("uid", "service:generateUID"));

		ObjectWrapper wrapper = (ObjectWrapper) tRoot.context.get("PREQueryTrigger" + tNode.class.getName());
	...
	private static ToolEntry getDirectEditLabel() {
		...
		tool.setName("PREQueryTrigger edit label");
