...
public class EnumValueConfigurator implements NodeConfigurator {
	public static tNode create() {
		return new tNode(new EnumValueConfigurator());
	}
	...
	public static List<ToolEntry> getTools() {
		...
		ToolEntry tool = getEnumAttributeNodeCreationDescription();
		list.add(tool);
	...
	private static ToolEntry getEnumAttributeNodeCreationDescription() {
		NodeCreationDescription tool = ToolFactory.eINSTANCE.createNodeCreationDescription();
		tool.setName("EnumAttribute");
		...
		CreateInstance c2 = ToolHelper.createInstance("type.EnumAttribute", "instance", "values");
		c1.getSubModelOperations().add(c2);
		c2.getSubModelOperations().add(ToolHelper.createSet("uid", "service:generateUID"));

		ObjectWrapper wrapper = (ObjectWrapper) tRoot.context.get("EnumAttribute" + tNode.class.getName());
	...
	private static ToolEntry getDirectEditLabel() {
		...
		tool.setName("EnumAttribute edit label");
