...
public class RouterConfigurator implements NodeConfigurator {
	public static tNode create() {
		return new tNode(new RouterConfigurator());
	}
	...
	public NodeStyleDescription getStyle() {
		...
		style.setWorkspacePath("/org.tura.metamodel.sirius.diagram.designer/images/router.png");
	...
	private static ToolEntry getServerNodeCreationDescription() {
		...
		tool.setIconPath("/org.tura.metamodel.sirius.diagram.designer/icons/router.png");
		...
		CreateInstance c2 = ToolHelper.createInstance("infrastructure.Router", "instance", "infrastructureComponent");
		c1.getSubModelOperations().add(c2);
		c2.getSubModelOperations().add(ToolHelper.createSet("uid", "service:generateUID"));

		ObjectWrapper wrapper = (ObjectWrapper) tRoot.context.get("Router" + tNode.class.getName());
	...
	private static ToolEntry getDirectEditLabel() {
		...
		tool.setName("Router edit label");
