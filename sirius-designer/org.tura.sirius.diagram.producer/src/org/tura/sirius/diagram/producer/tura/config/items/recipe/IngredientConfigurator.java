...
public class IngredientConfigurator implements ContainerConfigurator {
	public static tContainer create() {
		return new tContainer(new IngredientConfigurator());
	}
	...
	public ContainerStyleDescription getStyle() {
		...
		SystemColor backgroundColor = EnvironmentSystemColorFactory.getDefault().getSystemColorDescription("yellow");
	...
	public static List<ToolEntry> getTools() {
		...
		ToolEntry tool = getIngredientContainerCreationDescription();
		list.add(tool);
	...
	private static ToolEntry getIngredientContainerCreationDescription() {
		...
		CreateInstance c2 = ToolHelper.createInstance("recipe.Ingredient", "instance", "ingredients");
		c1.getSubModelOperations().add(c2);
		c2.getSubModelOperations().add(ToolHelper.createSet("uid", "service:generateUID"));

		ObjectWrapper wrapper = (ObjectWrapper) tRoot.context.get("Ingredient" + tContainer.class.getName());
	...
	private static ToolEntry getDirectEditLabel() {
		...
		tool.setName("Ingredient edit label");
