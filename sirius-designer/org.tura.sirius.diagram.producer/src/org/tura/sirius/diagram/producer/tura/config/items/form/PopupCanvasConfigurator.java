...
public class PopupCanvasConfigurator implements ContainerConfigurator {
	public static tContainer create() {
		return new tContainer(new PopupCanvasConfigurator());
	}
	...
	public static List<ToolEntry> getTools() {
		...
		ToolEntry tool = getPopupCanvasContainerCreationDescription();
		list.add(tool);
	...
	private static ToolEntry getPopupCanvasContainerCreationDescription() {
		ContainerCreationDescription tool = ToolFactory.eINSTANCE.createContainerCreationDescription();
		tool.setName("PopupCanvas");
		...
		CreateInstance c1 = ToolHelper.createInstance("form.PopupCanvas", "instance", "canvases");
		v.setFirstModelOperations(c1);
		c1.getSubModelOperations().add(ToolHelper.createSet("uid", "service:generateUID"));

		ObjectWrapper wrapper = (ObjectWrapper) tRoot.context.get("PopupCanvas" + tContainer.class.getName());
	...
	private static ToolEntry getDirectEditLabel() {
		...
		tool.setName("PopupCanvas edit label");
