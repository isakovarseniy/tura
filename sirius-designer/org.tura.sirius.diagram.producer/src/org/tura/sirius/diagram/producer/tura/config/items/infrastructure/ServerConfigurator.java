...
public class ServerConfigurator implements NodeConfigurator {
	public static tNode create() {
		return new tNode(new ServerConfigurator());
	}
	...
	public NodeStyleDescription getStyle() {
		...
		style.setWorkspacePath("/org.tura.metamodel.sirius.diagram.designer/images/server.png");
	...
	private static ToolEntry getServerNodeCreationDescription() {
		...
		tool.setIconPath("/org.tura.metamodel.sirius.diagram.designer/icons/server.png");
		...
		CreateInstance c2 = ToolHelper.createInstance("infrastructure.Server", "instance", "infrastructureComponent");
		c1.getSubModelOperations().add(c2);
		c2.getSubModelOperations().add(ToolHelper.createSet("uid", "service:generateUID"));

		ObjectWrapper wrapper = (ObjectWrapper) tRoot.context.get("Server" + tNode.class.getName());
