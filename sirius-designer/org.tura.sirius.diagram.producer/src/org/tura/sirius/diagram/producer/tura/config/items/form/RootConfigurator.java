...
public class RootConfigurator implements ContainerConfigurator {
	public static tContainer create() {
		return new tContainer(new RootConfigurator());
	}
	...
	public static List<ToolEntry> getTools() {
		...
		ToolEntry tool = getRootContainerCreationDescription();
		list.add(tool);
	...
	private static ToolEntry getRootContainerCreationDescription() {
		...
		CreateInstance c1 = ToolHelper.createInstance("form.Root", "instance", "root");
		v.setFirstModelOperations(c1);
		c1.getSubModelOperations().add(ToolHelper.createSet("uid", "service:generateUID"));

		ObjectWrapper wrapper = (ObjectWrapper) tRoot.context.get("Root" + tContainer.class.getName());
