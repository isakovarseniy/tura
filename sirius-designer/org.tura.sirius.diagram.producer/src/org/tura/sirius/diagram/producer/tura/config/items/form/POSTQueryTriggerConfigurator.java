...
public class POSTQueryTriggerConfigurator implements NodeConfigurator {
	public static tNode create() {
		return new tNode(new POSTQueryTriggerConfigurator());
	}
	...
	public String getCandidates() {
		return "feature:postQueryTrigger";
	}
	...
	public static List<ToolEntry> getTools() {
		...
		ToolEntry tool = getPOSTQueryTriggerNodeCreationDescription();
		list.add(tool);
	...
	private static ToolEntry getPOSTQueryTriggerNodeCreationDescription() {
		NodeCreationDescription tool = ToolFactory.eINSTANCE.createNodeCreationDescription();
		tool.setName("POSTQueryTrigger");
		...
		CreateInstance c2 = ToolHelper.createInstance("form.POSTQueryTrigger", "instance", "postQueryTrigger");
		c1.getSubModelOperations().add(c2);
		c2.getSubModelOperations().add(ToolHelper.createSet("uid", "service:generateUID"));

		ObjectWrapper wrapper = (ObjectWrapper) tRoot.context.get("POSTQueryTrigger" + tNode.class.getName());
	...
	private static ToolEntry getDirectEditLabel() {
		...
		tool.setName("POSTQueryTrigger edit label");
