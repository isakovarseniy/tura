...
public class SelectManyMenuConfigurator implements NodeConfigurator {
	private String suffix = "";
	private String feature = null;

	public SelectManyMenuConfigurator(String suffix, String feature) {
		this.suffix = suffix;
		this.feature = feature;
	}

	public SelectManyMenuConfigurator() {
	}

	public static tNode create(String suffix, String feature) {
		return new tNode(new SelectManyMenuConfigurator(suffix, feature));
	}
	...
	public static tNode create(String suffix) {
		return new tNode(new SelectManyMenuConfigurator(suffix, null));
	}

	public static tNode create() {
		return new tNode(new SelectManyMenuConfigurator());
	}

	public String getName() {
		return "SelectManyMenu" + this.suffix;
	}
	...
	public NodeStyleDescription getStyle() {
		...
		style.setWorkspacePath("/org.tura.metamodel.sirius.diagram.designer/images/selectmanymenu.png");
	...
	public static List<ToolEntry> getTools() {
		...
		ToolEntry tool = getSelectManyMenuNodeCreationDescription();
		list.add(tool);
	...
	private static ToolEntry getSelectManyMenuNodeCreationDescription() {
		NodeCreationDescription tool = ToolFactory.eINSTANCE.createNodeCreationDescription();
		tool.setName("SelectManyMenu");
		...
		tool.setIconPath("/org.tura.metamodel.sirius.diagram.designer/icons/selectmanymenu.png");
		...
		CreateInstance c3 = ToolHelper.createInstance("form.SelectManyMenu", "instance", "children");
		...
		c1.getCases().add(c2);
		c3 = ToolHelper.createInstance("form.SelectManyMenu", "instance", "children");
		c2.getSubModelOperations().add(c3);
		c3.getSubModelOperations().add(ToolHelper.createSet("uid", "service:generateUID"));
		...
		c3 = ToolHelper.createInstance("form.SelectManyMenu", "instance", "element");
		c2.getSubModelOperations().add(c3);
		c3.getSubModelOperations().add(ToolHelper.createSet("uid", "service:generateUID"));
		for (int i = 0; i < CanvasDiagram.SUFFIX_LIMIT; i++) {
			ObjectWrapper wrapper = (ObjectWrapper) tRoot.context.get("SelectManyMenu" + i + tNode.class.getName());
	...
	private static ToolEntry getDirectEditLabel() {
		...
		tool.setName("SelectManyMenu edit label");
	...
	private static ToolEntry getDrugAndDrop() {
		...
		tool.setName("SelectManyMenu to LayerHolder");
