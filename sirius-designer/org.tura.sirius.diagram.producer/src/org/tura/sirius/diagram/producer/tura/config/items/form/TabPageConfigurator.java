...
public class TabPageConfigurator implements ContainerConfigurator {
	public static tContainer create() {
		return new tContainer(new TabPageConfigurator());
	}
	...
	public static List<ToolEntry> getTools() {
		...
		ToolEntry tool = getTabPageContainerCreationDescription();
		list.add(tool);
	...
	private static ToolEntry getTabPageContainerCreationDescription() {
		...
		CreateInstance c1 = ToolHelper.createInstance("form.TabPage", "instance", "canvases");
		v.setFirstModelOperations(c1);
		c1.getSubModelOperations().add(ToolHelper.createSet("uid", "service:generateUID"));

		ObjectWrapper wrapper = (ObjectWrapper) tRoot.context.get("TabPage" + tContainer.class.getName());
	...
	private static ToolEntry getDirectEditLabel() {
		...
		tool.setName("TabPage edit label");
