...
public class DataExporterConfigurator implements NodeConfigurator {
	private String suffix = "";
	private String feature = null;

	public DataExporterConfigurator(String suffix, String feature) {
		this.suffix = suffix;
		this.feature = feature;
	}

	public DataExporterConfigurator() {
	}

	public static tNode create(String suffix, String feature) {
		return new tNode(new DataExporterConfigurator(suffix, feature));
	}
	...
	public static tNode create(String suffix) {
		return new tNode(new DataExporterConfigurator(suffix, null));
	}

	public static tNode create() {
		return new tNode(new DataExporterConfigurator());
	}

	public String getName() {
		return "DataExporter" + this.suffix;
	}
	...
	public NodeStyleDescription getStyle() {
		...
		style.setWorkspacePath("/org.tura.metamodel.sirius.diagram.designer/images/dataExporter.png");
	...
	public static List<ToolEntry> getTools() {
		...
		ToolEntry tool = getDataExporterNodeCreationDescription();
		list.add(tool);
	...
	private static ToolEntry getDataExporterNodeCreationDescription() {
		NodeCreationDescription tool = ToolFactory.eINSTANCE.createNodeCreationDescription();
		tool.setName("DataExporter");
		...
		tool.setIconPath("/org.tura.metamodel.sirius.diagram.designer/icons/dataExporter.png");
		...
		CreateInstance c3 = ToolHelper.createInstance("form.DataExporter", "instance", "children");
		...
		c1.getCases().add(c2);
		c3 = ToolHelper.createInstance("form.DataExporter", "instance", "children");
		c2.getSubModelOperations().add(c3);
		c3.getSubModelOperations().add(ToolHelper.createSet("uid", "service:generateUID"));
		...
		c3 = ToolHelper.createInstance("form.DataExporter", "instance", "element");
		c2.getSubModelOperations().add(c3);
		c3.getSubModelOperations().add(ToolHelper.createSet("uid", "service:generateUID"));
		...
		for (int i = 0; i < CanvasDiagram.SUFFIX_LIMIT; i++) {
			ObjectWrapper wrapper = (ObjectWrapper) tRoot.context.get("DataExporter" + i + tNode.class.getName());
	...
	private static ToolEntry getDirectEditLabel() {
		...
		tool.setName("DataExporter edit label");
