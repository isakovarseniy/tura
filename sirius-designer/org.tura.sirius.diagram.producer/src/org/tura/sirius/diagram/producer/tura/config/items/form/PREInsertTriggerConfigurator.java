...
public class PREInsertTriggerConfigurator implements NodeConfigurator {
	public static tNode create() {
		return new tNode(new PREInsertTriggerConfigurator());
	}
	...
	public String getCandidates() {
		return "feature:preInsertTrigger";
	}
	...
	public static List<ToolEntry> getTools() {
		...
		ToolEntry tool = getPREInsertTriggerNodeCreationDescription();
		list.add(tool);
	...
	private static ToolEntry getPREInsertTriggerNodeCreationDescription() {
		NodeCreationDescription tool = ToolFactory.eINSTANCE.createNodeCreationDescription();
		tool.setName("PREInsertTrigger");
		...
		CreateInstance c2 = ToolHelper.createInstance("form.PREInsertTrigger", "instance", "preInsertTrigger");
		c1.getSubModelOperations().add(c2);
		c2.getSubModelOperations().add(ToolHelper.createSet("uid", "service:generateUID"));

		ObjectWrapper wrapper = (ObjectWrapper) tRoot.context.get("PREInsertTrigger" + tNode.class.getName());
	...
	private static ToolEntry getDirectEditLabel() {
		...
		tool.setName("PREInsertTrigger edit label");
