...
public class PREFormTriggerConfigurator implements NodeConfigurator {
	public static tNode create() {
		return new tNode(new PREFormTriggerConfigurator());
	}
	...
	public static List<ToolEntry> getTools() {
		...
		ToolEntry tool = getPREFormTriggerNodeCreationDescription();
		list.add(tool);
	...
	private static ToolEntry getPREFormTriggerNodeCreationDescription() {
		NodeCreationDescription tool = ToolFactory.eINSTANCE.createNodeCreationDescription();
		tool.setName("PREFormTrigger");
		...
		CreateInstance c2 = ToolHelper.createInstance("form.PREFormTrigger", "instance", "preFormTrigger");
		c1.getSubModelOperations().add(c2);
		c2.getSubModelOperations().add(ToolHelper.createSet("uid", "service:generateUID"));

		ObjectWrapper wrapper = (ObjectWrapper) tRoot.context.get("PREFormTrigger" + tNode.class.getName());
	...
	private static ToolEntry getDirectEditLabel() {
		...
		tool.setName("PREFormTrigger edit label");
