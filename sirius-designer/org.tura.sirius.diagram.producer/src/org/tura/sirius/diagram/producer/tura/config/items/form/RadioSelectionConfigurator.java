...
public class RadioSelectionConfigurator implements NodeConfigurator {
	private String suffix = "";
	private String feature = null;

	public RadioSelectionConfigurator(String suffix, String feature) {
		this.suffix = suffix;
		this.feature = feature;
	}

	public RadioSelectionConfigurator() {
	}

	public static tNode create(String suffix, String feature) {
		return new tNode(new RadioSelectionConfigurator(suffix, feature));
	}
	...
	public static tNode create(String suffix) {
		return new tNode(new RadioSelectionConfigurator(suffix, null));
	}

	public static tNode create() {
		return new tNode(new RadioSelectionConfigurator());
	}

	public String getName() {
		return "RadioSelection" + this.suffix;
	}
	...
	public NodeStyleDescription getStyle() {
		...
		style.setWorkspacePath("/org.tura.metamodel.sirius.diagram.designer/images/radioSelection.png");
	...
	public static List<ToolEntry> getTools() {
		...
		ToolEntry tool = getRadioSelectionNodeCreationDescription();
		list.add(tool);
	...
	private static ToolEntry getRadioSelectionNodeCreationDescription() {
		...
		tool.setIconPath("/org.tura.metamodel.sirius.diagram.designer/icons/radioSelection.png");
		...
		CreateInstance c3 = ToolHelper.createInstance("form.RadioSelection", "instance", "children");
		...
		c1.getCases().add(c2);
		c3 = ToolHelper.createInstance("form.RadioSelection", "instance", "children");
		c2.getSubModelOperations().add(c3);
		c3.getSubModelOperations().add(ToolHelper.createSet("uid", "service:generateUID"));
		...
		c3 = ToolHelper.createInstance("form.RadioSelection", "instance", "element");
		c2.getSubModelOperations().add(c3);
		c3.getSubModelOperations().add(ToolHelper.createSet("uid", "service:generateUID"));
		for (int i = 0; i < CanvasDiagram.SUFFIX_LIMIT; i++) {
			ObjectWrapper wrapper = (ObjectWrapper) tRoot.context.get("RadioSelection" + i + tNode.class.getName());
	...
	private static ToolEntry getDirectEditLabel() {
		...
		tool.setName("RadioSelection edit label");
	...
	private static ToolEntry getDrugAndDrop() {
		...
		tool.setName("Radio to LayerHolder");
		...
		InitialContainerDropOperation opr = ToolHelper.createInitialRadioOperation();
		tool.setInitialOperation(opr);
