...
public class DataTemplateConfigurator implements NodeConfigurator {
	private String suffix = "";
	private String feature = null;

	public DataTemplateConfigurator(String suffix, String feature) {
		this.suffix = suffix;
		this.feature = feature;
	}

	public DataTemplateConfigurator() {
	}

	public static tNode create(String suffix, String feature) {
		return new tNode(new DataTemplateConfigurator(suffix, feature));
	}

	public static tNode create(String suffix) {
		return new tNode(new DataTemplateConfigurator(suffix, null));
	}
	...
	public static tNode create() {
		return new tNode(new DataTemplateConfigurator());
	}

	public String getName() {
		return "DataTemplate" + this.suffix;
	}
	...
	public NodeStyleDescription getStyle() {
		...
		style.setWorkspacePath("/org.tura.metamodel.sirius.diagram.designer/images/datatemplate.png");
	...
	public static List<ToolEntry> getTools() {
		...
		ToolEntry tool = getDataTemplateNodeCreationDescription();
		list.add(tool);
	...
	private static ToolEntry getDataTemplateNodeCreationDescription() {
		...
		tool.setIconPath("/org.tura.metamodel.sirius.diagram.designer/icons/datatemplate.png");
		...
		CreateInstance c3 = ToolHelper.createInstance("form.DataTemplate", "instance", "children");
		...
		c1.getCases().add(c2);
		c3 = ToolHelper.createInstance("form.DataTemplate", "instance", "children");
		c2.getSubModelOperations().add(c3);
		c3.getSubModelOperations().add(ToolHelper.createSet("uid", "service:generateUID"));
		...
		c3 = ToolHelper.createInstance("form.DataTemplate", "instance", "element");
		c2.getSubModelOperations().add(c3);
		c3.getSubModelOperations().add(ToolHelper.createSet("uid", "service:generateUID"));
		for (int i = 0; i < CanvasDiagram.SUFFIX_LIMIT; i++) {
			ObjectWrapper wrapper = (ObjectWrapper) tRoot.context.get("DataTemplate" + i + tNode.class.getName());
	...
	private static ToolEntry getDirectEditLabel() {
		...
		tool.setName("DataTemplate edit label");
	...
	private static ToolEntry getDrugAndDrop() {
		...
		tool.setName("DataTemplate to LayerHolder");
