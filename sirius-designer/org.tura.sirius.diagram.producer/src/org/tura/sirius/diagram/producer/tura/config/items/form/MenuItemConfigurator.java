...
public class MenuItemConfigurator implements NodeConfigurator {
	public static tNode create() {
		return new tNode(new MenuItemConfigurator());
	}
	...
	public static List<ToolEntry> getTools() {
		...
		ToolEntry tool = getMenuItemNodeCreationDescription();
		list.add(tool);
	...
	private static ToolEntry getMenuItemNodeCreationDescription() {
		NodeCreationDescription tool = ToolFactory.eINSTANCE.createNodeCreationDescription();
		tool.setName("MenuItem");
		...
		CreateInstance c2 = ToolHelper.createInstance("form.MenuItem", "instance", "menuElements");
		c1.getSubModelOperations().add(c2);
		c2.getSubModelOperations().add(ToolHelper.createSet("uid", "service:generateUID"));

		ObjectWrapper wrapper = (ObjectWrapper) tRoot.context.get("MenuItem" + tNode.class.getName());
	...
	private static ToolEntry getDirectEditLabel() {
		...
		tool.setName("MenuItem edit label");
