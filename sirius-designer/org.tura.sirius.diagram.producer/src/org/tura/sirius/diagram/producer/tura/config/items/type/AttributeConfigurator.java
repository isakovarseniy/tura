...
public class AttributeConfigurator implements NodeConfigurator {
	public static tNode create() {
		return new tNode(new AttributeConfigurator());
	}
	...
	public static List<ToolEntry> getTools() {
		...
		ToolEntry tool = getAttributeNodeCreationDescription();
		list.add(tool);
	...
	private static ToolEntry getAttributeNodeCreationDescription() {
		NodeCreationDescription tool = ToolFactory.eINSTANCE.createNodeCreationDescription();
		tool.setName("Attribute");
		...
		CreateInstance c2 = ToolHelper.createInstance("type.Attribute", "instance", "attributes");
		c1.getSubModelOperations().add(c2);
		c2.getSubModelOperations().add(ToolHelper.createSet("uid", "service:generateUID"));

		ObjectWrapper wrapper = (ObjectWrapper) tRoot.context.get("Attribute" + tNode.class.getName());
	...
	private static ToolEntry getDirectEditLabel() {
		...
		tool.setName("Attribute edit label");
