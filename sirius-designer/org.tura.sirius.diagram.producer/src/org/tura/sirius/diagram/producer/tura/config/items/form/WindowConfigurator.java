...
public class WindowConfigurator implements ContainerConfigurator {
	public static tContainer create() {
		return new tContainer(new WindowConfigurator());
	}
	...
	public static List<ToolEntry> getTools() {
		...
		ToolEntry tool = getWindowContainerCreationDescription();
		list.add(tool);
	...
	private static ToolEntry getWindowContainerCreationDescription() {
		...
		CreateInstance c1 = ToolHelper.createInstance("form.Window", "instance", "canvases");
		v.setFirstModelOperations(c1);
		c1.getSubModelOperations().add(ToolHelper.createSet("uid", "service:generateUID"));

		ObjectWrapper wrapper = (ObjectWrapper) tRoot.context.get("Window" + tContainer.class.getName());
	...
	private static ToolEntry getDirectEditLabel() {
		...
		tool.setName("Window edit label");
