...
public class EnumeratorConfigurator implements ContainerConfigurator {
	public static tContainer create() {
		return new tContainer(new EnumeratorConfigurator());
	}
	...
	public static List<ToolEntry> getTools() {
		...
		ToolEntry tool = getEnumeratorContainerCreationDescription();
		list.add(tool);
	...
	private static ToolEntry getEnumeratorContainerCreationDescription() {
		...
		CreateInstance c1 = ToolHelper.createInstance("type.Enumerator", "instance", "types");
		v.setFirstModelOperations(c1);
		c1.getSubModelOperations().add(ToolHelper.createSet("uid", "service:generateUID"));

		ObjectWrapper wrapper = (ObjectWrapper) tRoot.context.get("Enumerator" + tContainer.class.getName());
	...
	private static ToolEntry getDirectEditLabel() {
		...
		tool.setName("Enumerator edit label");
