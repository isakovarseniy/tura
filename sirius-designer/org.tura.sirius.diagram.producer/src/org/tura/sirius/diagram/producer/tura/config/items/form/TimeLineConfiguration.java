...
public class TimeLineConfiguration implements ContainerConfigurator {
	private String suffix = "";

	public TimeLineConfiguration(String suffix) {
		this.suffix = suffix;
	}
	...
	public static tContainer create() {
		return new tContainer(new TimeLineConfiguration());
	}

	public static tContainer create(String suffix) {
		return new tContainer(new TimeLineConfiguration(suffix));
	}

	public String getName() {
		return "TimeLine" + this.suffix;
	}
	...
	public static List<ToolEntry> getTools() {
		...
		ToolEntry tool = getTimeLineContainerCreationDescription();
		list.add(tool);
	...
	private static ToolEntry getTimeLineContainerCreationDescription() {
		...
		CreateInstance c3 = ToolHelper.createInstance("form.TimeLine", "instance", "children");
		v.setFirstModelOperations(c3);
		c3.getSubModelOperations().add(ToolHelper.createSet("uid", "service:generateUID"));

		ObjectWrapper wrapper = (ObjectWrapper) tRoot.context.get("TimeLine" + tContainer.class.getName());
	...
	private static ToolEntry getDrugAndDrop() {
		...
		tool.setName("Table to TimeLine");
		...
		ObjectWrapper w = (ObjectWrapper) tRoot.context.get("TimeLine" + tContainer.class.getName());
