...
public class MenuDefinitionConfigurator implements NodeConfigurator {
	public static tNode create() {
		return new tNode(new MenuDefinitionConfigurator());
	}
	...
	public static List<ToolEntry> getTools() {
		...
		ToolEntry tool = getMenuDefinitionNodeCreationDescription();
		list.add(tool);
	...
	private static ToolEntry getMenuDefinitionNodeCreationDescription() {
		NodeCreationDescription tool = ToolFactory.eINSTANCE.createNodeCreationDescription();
		tool.setName("MenuDefinition");
		...
		CreateInstance c2 = ToolHelper.createInstance("form.MenuDefinition", "instance", "menus");
		c1.getSubModelOperations().add(c2);
		c2.getSubModelOperations().add(ToolHelper.createSet("uid", "service:generateUID"));

		ObjectWrapper wrapper = (ObjectWrapper) tRoot.context.get("MenuDefinition" + tNode.class.getName());
	...
	private static ToolEntry getDirectEditLabel() {
		...
		tool.setName("MenuDefinition edit label");
