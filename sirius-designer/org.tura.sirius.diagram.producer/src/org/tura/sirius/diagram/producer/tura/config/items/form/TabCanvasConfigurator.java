...
public class TabCanvasConfigurator implements NodeConfigurator {
	public static tNode create() {
		return new tNode(new TabCanvasConfigurator());
	}
	...
	public static List<ToolEntry> getTools() {
		...
		ToolEntry tool = getTabCanvasNodeCreationDescription();
		list.add(tool);
	...
	private static ToolEntry getTabCanvasNodeCreationDescription() {
		NodeCreationDescription tool = ToolFactory.eINSTANCE.createNodeCreationDescription();
		tool.setName("TabCanvas");
		...
		CreateInstance c2 = ToolHelper.createInstance("form.TabCanvas", "instance", "canvases");
		c1.getSubModelOperations().add(c2);
		c2.getSubModelOperations().add(ToolHelper.createSet("uid", "service:generateUID"));

		ObjectWrapper wrapper = (ObjectWrapper) tRoot.context.get("TabCanvas" + tNode.class.getName());
	...
	private static ToolEntry getDirectEditLabel() {
		...
		tool.setName("TabCanvas edit label");
