...
public class MenuExtensionPointConfigurator implements NodeConfigurator {
	public static tNode create() {
		return new tNode(new MenuExtensionPointConfigurator());
	}
	...
	public String getDomainClass() {
		return "form.MenuExtensionPoint";
	}
	...
	public NodeStyleDescription getStyle() {
		...
		SystemColor foregroundColor = EnvironmentSystemColorFactory.getDefault().getSystemColorDescription("dark_blue");
	...
	public static List<ToolEntry> getTools() {
		...
		ToolEntry tool = getMenuExtensionPointNodeCreationDescription();
		list.add(tool);
	...
	private static ToolEntry getMenuExtensionPointNodeCreationDescription() {
		NodeCreationDescription tool = ToolFactory.eINSTANCE.createNodeCreationDescription();
		tool.setName("MenuExtensionPoint");
		...
		CreateInstance c2 = ToolHelper.createInstance("form.MenuExtensionPoint", "instance", "menuElements");
		c1.getSubModelOperations().add(c2);
		c2.getSubModelOperations().add(ToolHelper.createSet("uid", "service:generateUID"));

		ObjectWrapper wrapper = (ObjectWrapper) tRoot.context.get("MenuExtensionPoint" + tNode.class.getName());
	...
	private static ToolEntry getDirectEditLabel() {
		...
		tool.setName("MenuExtensionPoint edit label");
