...
public class OutputTextConfigurator implements NodeConfigurator {
	private String suffix = "";
	private String feature = null;

	public OutputTextConfigurator(String suffix, String feature) {
		this.suffix = suffix;
		this.feature = feature;
	}

	public OutputTextConfigurator() {
	}

	public static tNode create(String suffix) {
		return new tNode(new OutputTextConfigurator(suffix, null));
	}
	...
	public static tNode create(String suffix, String feature) {
		return new tNode(new OutputTextConfigurator(suffix, feature));
	}

	public static tNode create() {
		return new tNode(new OutputTextConfigurator());
	}

	public String getName() {
		return "OutputText" + this.suffix;
	}
	...
	public NodeStyleDescription getStyle() {
		...
		style.setWorkspacePath("/org.tura.metamodel.sirius.diagram.designer/images/outputtext.png");
	...
	public static List<ToolEntry> getTools() {
		...
		ToolEntry tool = getOutputTextNodeCreationDescription();
		list.add(tool);
	...
	private static ToolEntry getOutputTextNodeCreationDescription() {
		...
		tool.setIconPath("/org.tura.metamodel.sirius.diagram.designer/icons/outputtext.png");
		...
		CreateInstance c3 = ToolHelper.createInstance("form.OutputText", "instance", "children");
		...
		c1.getCases().add(c2);
		c3 = ToolHelper.createInstance("form.OutputText", "instance", "children");
		c2.getSubModelOperations().add(c3);
		c3.getSubModelOperations().add(ToolHelper.createSet("uid", "service:generateUID"));
		...
		c3 = ToolHelper.createInstance("form.OutputText", "instance", "element");
		c2.getSubModelOperations().add(c3);
		c3.getSubModelOperations().add(ToolHelper.createSet("uid", "service:generateUID"));
		for (int i = 0; i < CanvasDiagram.SUFFIX_LIMIT; i++) {
			ObjectWrapper wrapper = (ObjectWrapper) tRoot.context.get("OutputText" + i + tNode.class.getName());
	...
	private static ToolEntry getDirectEditLabel() {
		...
		tool.setName("OutputText edit label");
	...
	private static ToolEntry getDrugAndDrop() {
		...
		tool.setName("OutputText to LayerHolder");
