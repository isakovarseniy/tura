...
public class SubMenuConfigurator implements NodeConfigurator {
	public static tNode create() {
		return new tNode(new SubMenuConfigurator());
	}
	...
	public NodeStyleDescription getStyle() {
		...
		SystemColor foregroundColor = EnvironmentSystemColorFactory.getDefault().getSystemColorDescription("blue");
	...
	public static List<ToolEntry> getTools() {
		...
		ToolEntry tool = getSubMenuNodeCreationDescription();
		list.add(tool);
	...
	private static ToolEntry getSubMenuNodeCreationDescription() {
		...
		CreateInstance c2 = ToolHelper.createInstance("form.SubMenu", "instance", "menuElements");
		c1.getSubModelOperations().add(c2);
		c2.getSubModelOperations().add(ToolHelper.createSet("uid", "service:generateUID"));

		ObjectWrapper wrapper = (ObjectWrapper) tRoot.context.get("SubMenu" + tNode.class.getName());
	...
	private static ToolEntry getDirectEditLabel() {
		...
		tool.setName("SubMenu edit label");
