...
public class ArtifactConfigurator implements ContainerConfigurator {
	public static tContainer create() {
		return new tContainer(new ArtifactConfigurator());
	}
	...
	private static ToolEntry getTypeContainerCreationDescription() {
		...
		CreateInstance c1 = ToolHelper.createInstance("artifact.Artifact", "instance", "artifacts");
		v.setFirstModelOperations(c1);
		c1.getSubModelOperations().add(ToolHelper.createSet("uid", "service:generateUID"));

		ObjectWrapper wrapper = (ObjectWrapper) tRoot.context.get("Artifact" + tContainer.class.getName());
	...
	private static ToolEntry getDirectEditLabel() {
		...
		tool.setName("Artifact edit label");
