...
public class PREDeleteTriggerConfigurator implements NodeConfigurator {
	public static tNode create() {
		return new tNode(new PREDeleteTriggerConfigurator());
	}
	...
	public String getCandidates() {
		return "feature:preDeleteTrigger";
	}
	...
	public static List<ToolEntry> getTools() {
		...
		ToolEntry tool = getPREDeleteTriggerNodeCreationDescription();
		list.add(tool);
	...
	private static ToolEntry getPREDeleteTriggerNodeCreationDescription() {
		NodeCreationDescription tool = ToolFactory.eINSTANCE.createNodeCreationDescription();
		tool.setName("PREDeleteTrigger");
		...
		CreateInstance c2 = ToolHelper.createInstance("form.PREDeleteTrigger", "instance", "preDeleteTrigger");
		c1.getSubModelOperations().add(c2);
		c2.getSubModelOperations().add(ToolHelper.createSet("uid", "service:generateUID"));

		ObjectWrapper wrapper = (ObjectWrapper) tRoot.context.get("PREDeleteTrigger" + tNode.class.getName());
	...
	private static ToolEntry getDirectEditLabel() {
		...
		tool.setName("PREDeleteTrigger edit label");
