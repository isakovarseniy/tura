...
public class HubConfigurator implements NodeConfigurator {
	public static tNode create() {
		return new tNode(new HubConfigurator());
	}
	...
	public NodeStyleDescription getStyle() {
		...
		style.setWorkspacePath("/org.tura.metamodel.sirius.diagram.designer/images/hub.png");
	...
	private static ToolEntry getServerNodeCreationDescription() {
		...
		tool.setIconPath("/org.tura.metamodel.sirius.diagram.designer/icons/hub.png");
		...
		CreateInstance c2 = ToolHelper.createInstance("infrastructure.Hub", "instance", "infrastructureComponent");
		c1.getSubModelOperations().add(c2);
		c2.getSubModelOperations().add(ToolHelper.createSet("uid", "service:generateUID"));

		ObjectWrapper wrapper = (ObjectWrapper) tRoot.context.get("Hub" + tNode.class.getName());
