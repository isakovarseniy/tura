...
public class OverlayCanvasConfigurator implements ContainerConfigurator {
	public static tContainer create() {
		return new tContainer(new OverlayCanvasConfigurator());
	}
	...
	public String getCandidates() {
		return "aql:self.canvases";
	}

	public String getDomainClass() {
		return "form.OverlayCanvas";
	}
	...
	public static List<ToolEntry> getTools() {
		...
		ToolEntry tool = getOverlayCanvasContainerCreationDescription();
		list.add(tool);
	...
	private static ToolEntry getOverlayCanvasContainerCreationDescription() {
		ContainerCreationDescription tool = ToolFactory.eINSTANCE.createContainerCreationDescription();
		tool.setName("OverlayCanvas");
		...
		CreateInstance c1 = ToolHelper.createInstance("form.OverlayCanvas", "instance", "canvases");
		v.setFirstModelOperations(c1);
		c1.getSubModelOperations().add(ToolHelper.createSet("uid", "service:generateUID"));

		ObjectWrapper wrapper = (ObjectWrapper) tRoot.context.get("OverlayCanvas" + tContainer.class.getName());
	...
	private static ToolEntry getDirectEditLabel() {
		...
		tool.setName("OverlayCanvas edit label");
