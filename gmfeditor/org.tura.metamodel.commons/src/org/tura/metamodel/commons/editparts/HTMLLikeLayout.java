	...
	protected Dimension calculateLayout(IFigure parent) {
		...
		for (int i = 0; i < grid.size(); i++) {
			...
			for (int j = 0; j < row.size(); j++) {
				colWidth[j] = Math.max(colWidth[j],
						row.get(j).getBounds().width);
				rowHigh[i] = Math
						.max(rowHigh[i], row.get(j).getBounds().height);
			}
		}
		...
		for (int i = 0; i < grid.size(); i++) {
			...
			xOffset = 0;
			for (int j = 0; j < row.size(); j++) {
				Rectangle bounds = new Rectangle(xOffset, yOffset, row.get(j)
						.getBounds().width, row.get(j).getBounds().height);
