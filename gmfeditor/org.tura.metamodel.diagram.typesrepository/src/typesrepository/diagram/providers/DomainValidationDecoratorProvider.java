...
import typesrepository.diagram.part.DomainDiagramEditor;
...
public class DomainValidationDecoratorProvider extends AbstractProvider implements IDecoratorProvider {
	...
	public void createDecorators(IDecoratorTarget decoratorTarget) {
		...
		if (editPart instanceof GraphicalEditPart || editPart instanceof AbstractConnectionEditPart) {
			...
			if (((DiagramEditDomain) ed).getEditorPart() instanceof DomainDiagramEditor) {
	...
	public boolean provides(IOperation operation) {
		...
		return view != null && TypesRepositoryEditPart.MODEL_ID.equals(DomainVisualIDRegistry.getModelID(view));
	}
		...
		PlatformUI.getWorkbench().getDisplay().asyncExec(new Runnable() {
			...
			public void run() {
				try {
					...
					DomainDiagramEditorPlugin.getInstance().logError("Decorator refresh failure", e); //$NON-NLS-1$
				}
			}
		});
	}
	...
	public static class StatusDecorator extends AbstractDecorator {
		...
		public StatusDecorator(IDecoratorTarget decoratorTarget) {
			...
			try {
				...
				DomainDiagramEditorPlugin.getInstance().logError("ViewID access failure", e); //$NON-NLS-1$			
			}
		}
		...
		public void refresh() {
			...
			try {
				...
				DomainDiagramEditorPlugin.getInstance().logError("Validation markers refresh failure", e); //$NON-NLS-1$
			}
	...
	static class MarkerObserver implements IFileObserver {
		...
		private String getType(IMarker marker) {
			try {
				...
				DomainDiagramEditorPlugin.getInstance().logError("Validation marker refresh failure", e); //$NON-NLS-1$
				return ""; //$NON-NLS-1$
			}
		}
	}
