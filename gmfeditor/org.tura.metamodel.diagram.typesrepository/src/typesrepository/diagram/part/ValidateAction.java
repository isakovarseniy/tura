...
import typesrepository.diagram.providers.DomainMarkerNavigationProvider;
import typesrepository.diagram.providers.DomainValidationProvider;
...
public class ValidateAction extends Action {
	...
	public void run() {
		...
		if (workbenchPart instanceof IDiagramWorkbenchPart) {
			...
			try {
				...
				DomainDiagramEditorPlugin.getInstance().logError("Validation action failed", e); //$NON-NLS-1$
			}
		}
	}
	...
	public static void runValidation(View view) {
		try {
			//			if (typesrepository.diagram.part.DomainDiagramEditorUtil.openDiagram(view.eResource())) {
	...
	private static void createMarkers(IFile target, IStatus validationStatus, DiagramEditPart diagramEditPart) {
		...
		for (Iterator it = allStatuses.iterator(); it.hasNext();) {
			...
			View view = DomainDiagramEditorUtil.findView(diagramEditPart, nextStatus.getTarget(), element2ViewMap);
	...
	private static void createMarkers(IFile target, Diagnostic emfValidationStatus, DiagramEditPart diagramEditPart) {
		...
		for (Iterator it = emfValidationStatus.getChildren().iterator(); it.hasNext();) {
			...
			if (data != null && !data.isEmpty() && data.get(0) instanceof EObject) {
				...
				hash.put(EMFCoreUtil.getProxyID(element) + nextDiagnostic.getMessage(), nextDiagnostic);
			}
		}
		...
		for (Iterator it = hash.values().iterator(); it.hasNext();) {
			...
			if (data != null && !data.isEmpty() && data.get(0) instanceof EObject) {
				EObject element = (EObject) data.get(0);
				View view = DomainDiagramEditorUtil.findView(diagramEditPart, element, element2ViewMap);
	...
	private static void addMarker(EditPartViewer viewer, IFile target, String elementId, String location,
		...
		DomainMarkerNavigationProvider.addMarker(target, elementId, location, message, statusSeverity);
	}
