	...
	public List<Object> queryRows() {
		...
		try {
			...
			Object[] result = (new QueryHelper())
					.findRefreshedAeas((domain.MenuItem) property.getModel());
			...
			for (domain.AreaRef p : ((domain.MenuItem) property.getModel()).getRefreshAreas() ) {
				RefreshAreaRow r = new RefreshAreaRow(p);
				if (p.getArea().equals(((domain.MenuItem) property.getModel())) ){
					noThisElement = false;
					r.setTHIS("this");
				}
