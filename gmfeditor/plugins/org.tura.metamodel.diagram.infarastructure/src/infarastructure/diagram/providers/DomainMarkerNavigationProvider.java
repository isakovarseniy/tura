...
import infarastructure.diagram.part.DomainDiagramEditorUtil;
	...
	*/
	protected void doGotoMarker(IMarker marker) {
		...
		if (targetEditPart != null) {
			DomainDiagramEditorUtil.selectElementsInDiagram(editor, Arrays.asList(new EditPart[] { targetEditPart }));
		}
	}
	...
	*/
	public static void deleteMarkers(IResource resource) {
		try {
			...
			DomainDiagramEditorPlugin.getInstance().logError("Failed to delete validation markers", e); //$NON-NLS-1$
		}
	}
	...
	*/
	public static IMarker addMarker(IFile file, String elementId, String location, String message, int statusSeverity) {
		...
		try {
			...
			DomainDiagramEditorPlugin.getInstance().logError("Failed to create validation marker", e); //$NON-NLS-1$
		}
		return marker;
	}
}
