 ...
 * @generated
 */
public class TypesItemSemanticEditPolicy extends TuraBaseItemSemanticEditPolicy {
	...
	private void addDestroyChildNodesCommand(ICompositeCommand cmd) {
		...
		for (Iterator<?> nit = view.getChildren().iterator(); nit.hasNext();) {
			...
			switch (TuraVisualIDRegistry.getVisualID(node)) {
			case TypesTypesPrimitivesCompartmentEditPart.VISUAL_ID:
				for (Iterator<?> cit = node.getChildren().iterator(); cit
						.hasNext();) {
					Node cnode = (Node) cit.next();
					switch (TuraVisualIDRegistry.getVisualID(cnode)) {
