...
public abstract class BlockableDS extends DataSource {

	private GridProperty property;

	public BlockableDS(GridProperty property) {
	...
	protected List<Object> getContextRoot() {
		try {
			...
			if  ( property.getModel() instanceof Uielement) {
				  result = (new QueryHelper()).findRefreshedAeas((Uielement) property.getModel());
				}
				if  ( property.getModel() instanceof form.Window) {
					  result = (new QueryHelper()).findRefreshedAeas((form.Window) property.getModel());
					}
			...
			for (AreaRef p : ((Blockable) property.getModel()).getBlock()) {
				if (p.getGroup() == getGroup()) {
					RefreshAreaRow r = new RefreshAreaRow(p);
					if (p.getArea().equals(( property.getModel()))) {
						noThisElement = false;
						r.setTHIS("this");
					}
			...
			if (noThisElement && property.getModel() instanceof Uielement) {
	...
	public void addRow() {
		...
		if (dialog.open() == Window.OK) {
			...
			for(  AreaRef ref : ((Blockable) property.getModel()).getBlock()){
				if (ref.getGroup() != getGroup()){
					areaRef.add(ref);
				}
			}

			editingDomain.getCommandStack().execute(SetCommand.create(editingDomain, ((Blockable) property.getModel()),
					FormPackage.eINSTANCE.getBlockable_Block(), areaRef));
	...
	public List<Object> queryRows() {
		...
		try {
			...
			editingDomain.getCommandStack().execute(RemoveCommand.create(editingDomain, property.getModel(),
					FormPackage.eINSTANCE.getBlockable_Block(), removeAreas));
