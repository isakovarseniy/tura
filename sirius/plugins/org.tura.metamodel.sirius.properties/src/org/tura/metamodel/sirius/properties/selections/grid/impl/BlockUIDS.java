...
public abstract class BlockUIDS extends DataSource {

	private GridProperty property;

	public BlockUIDS(GridProperty property) {
	...
	protected List<Object> getContextRoot() {
		try {
			...
			for (AreaRef p : ((BlockUI) property.getModel()).getSourceTarget()) {
	...
	public void addRow() {
		...
		if (dialog.open() == Window.OK) {
			...
			for(  AreaRef ref : ((BlockUI) property.getModel()).getSourceTarget()){
				if (ref.getGroup() != getGroup()){
					areaRef.add(ref);
				}
			}

			editingDomain.getCommandStack().execute(SetCommand.create(editingDomain, ((BlockUI) property.getModel()),
					FormPackage.eINSTANCE.getBlockUI_SourceTarget(), areaRef));
	...
	public List<Object> queryRows() {
		...
		try {
			...
			editingDomain.getCommandStack().execute(RemoveCommand.create(editingDomain, property.getModel(),
					FormPackage.eINSTANCE.getBlockUI_SourceTarget(), removeAreas));
