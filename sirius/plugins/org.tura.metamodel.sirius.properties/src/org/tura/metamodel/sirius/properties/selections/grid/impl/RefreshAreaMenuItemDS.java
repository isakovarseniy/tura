...
public abstract class RefreshAreaMenuItemDS extends DataSource {

	private GridProperty property;

	public RefreshAreaMenuItemDS(GridProperty property) {
	...
	protected List<Object> getContextRoot() {
		try {
			...
			Object[] result = (new QueryHelper()).findRefreshedAeas((MenuItem) property.getModel());
			...
			if (noThisElement) {
				rows.add(new RefreshAreaRow(((MenuItem) property.getModel()) , "this"));
			}
	...
	public void addRow() {
		...
		if (dialog.open() == Window.OK) {
			...
			for(  AreaRef ref : ((MenuItem) property.getModel()).getRefreshAreas() ){
				if (ref.getGroup() != getGroup()){
					areaRef.add(ref);
				}
			}

			editingDomain.getCommandStack().execute(SetCommand.create(editingDomain, ((MenuItem) property.getModel()),
					FormPackage.eINSTANCE.getMenuItem_RefreshAreas(), areaRef));
	...
	public List<Object> queryRows() {
		...
		try {
			...
			editingDomain.getCommandStack().execute(RemoveCommand.create(editingDomain, property.getModel(),
					FormPackage.eINSTANCE.getMenuItem_RefreshAreas(), removeAreas));
