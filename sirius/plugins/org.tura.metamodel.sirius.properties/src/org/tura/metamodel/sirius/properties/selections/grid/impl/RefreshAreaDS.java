...
public abstract class RefreshAreaDS extends DataSource {

	private GridProperty property;

	public RefreshAreaDS(GridProperty property) {
	...
	protected List<Object> getContextRoot() {
		try {
			...
			for (AreaRef p : ((Uielement) property.getModel()).getRefreshAreas()) {
	...
	public void addRow() {
		...
		if (dialog.open() == Window.OK) {
			...
			for(  AreaRef ref : ((Uielement) property.getModel()).getRefreshAreas() ){
				if (ref.getGroup() != getGroup()){
					areaRef.add(ref);
				}
			}

			editingDomain.getCommandStack().execute(SetCommand.create(editingDomain, ((Uielement) property.getModel()),
					FormPackage.eINSTANCE.getUielement_RefreshAreas(), areaRef));
	...
	public List<Object> queryRows() {
		...
		try {
			...
			editingDomain.getCommandStack().execute(RemoveCommand.create(editingDomain, property.getModel(),
					FormPackage.eINSTANCE.getUielement_RefreshAreas(), removeAreas));
