...
public class RestWorkbenchClient implements WorkbenchClient {
    ...
    private RestWorkbenchClient(String appUrl, String token, boolean async,
                                int jobTimeoutSeconds, int projectJobTimeoutSeconds, int cloneRepoTimeoutSeconds) {
        this.async = async;

        final Client client = ClientBuilder.newClient();
        if (token != null ) {
            client.register(new Authenticator(token));
        }
     ...
     */
    public static WorkbenchClient createAsyncWorkbenchClient(String appUrl, String token) {
        return new RestWorkbenchClient(appUrl, token, true);
    }

    /**
     * Creates Business Central REST client which will wait for successful completion of each operation.
     */
    public static WorkbenchClient createWorkbenchClient(String appUrl, String token) {
        return new RestWorkbenchClient(appUrl, token, false);
    }
