...
import org.apache.felix.gogo.jline.command.helper.ClientRequestTimedOutException;
...
public class BusinessCentralCreateSpace implements Executable, SessionAware{
	...
	public Object execute() throws Exception {
		...
		SpaceRequest request = null;
		try {
		request =   client.createSpace(space, user);
		}catch(ClientRequestTimedOutException e) {
			if ( e.getJobResult().getResult().equals("Space with name ${space} already exists".replaceAll("\\$\\{space}", space))) {
				return null;
			}
		}
		
		String jobId = request.getJobId();
		new Repeater(1000L,"Space creation in process ") {
