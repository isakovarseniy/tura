...
public class LoadDrugDropForm {
	public void execute(RepositoryFactory rf, HashMap<String, Object> context)
		...
		Reader root = new CanvasParser()
				.XWTModelbuilder(
						new File(
								"/home/platform/GITRepo/tura/testcases/testform/DrugDropForm/src/DataDefinition.xwt"),
						null);
		root.buildAll(context, rf, root, program);

		String frmUUID = Reader.idMAP.get("DrugDropForm");

		root = new CanvasParser()
				.XWTModelbuilder(
						new File(
								"/home/platform/GITRepo/tura/testcases/testform/DrugDropForm/src/Canvas.xwt"),
						frmUUID);
		root.buildAll(context, rf, root, program);
		...
		root = new CanvasParser()
				.XWTModelbuilder(
						new File(
								"/home/platform/GITRepo/tura/testcases/testform/DrugDropForm/src/Window.xwt"),
						frmUUID);
		root.buildAll(context, rf, root, program);

		Iterator<CommandDAO> itr = program.iterator();
		HashMap<String, Object> ctx = new Processor().executor(rf, itr);
		Form frm = (Form) ctx.get("DrugDropForm");
		...
		UIContainerDAO uicontainer = uch.addUIContainer("DrugDropForm");
