...
import org.tura.model.designer.ui.tree.selection.mmdesigner.treeselection.datacontrol.IBeanFactory;
...
@CustomQuery("mapper")
public class CustomMapperQueryTrigger implements QueryTrigger {
	...
	public <T> List<T> execute(CpaRepository repository, Pager<T> pager) throws TuraException {
		try {
			Method m = Reflection.findMethod(pager, "getParentObject");
			m.setAccessible(true);
			ApplicationMapper applicationMapper = (ApplicationMapper)m.invoke(pager);
			return (List<T>) applicationMapper.getMapper().getMappers();
		} catch (Exception e) {
