...
@CustomQuery("modelQuery")
public class CustomModelQueryQueryTrigger implements QueryTrigger{
	...
	public <T> List<T> execute(CpaRepository repository, Pager<T> pager) throws TuraException {
        ...
        Artifact  artifact =  modelMapper.getArtifactRef();
        if ( artifact != null) {
        	return (List<T>) artifact.getModelQuery();
        }
