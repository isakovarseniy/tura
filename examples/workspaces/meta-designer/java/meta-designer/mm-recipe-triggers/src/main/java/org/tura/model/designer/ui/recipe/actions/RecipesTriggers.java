...
public class RecipesTriggers implements OnChangeArtificialFieldTrigger, Crud {
	...
	public void createChildren(TreeDataControl tdc, DataControl<?> dc, ObjectControl oc, String childType) throws TuraException {
        if ( PreFormTrigger.RECIPE_CONSTANT.equals(childType)) {
    		String rel = "recipes2recipe";
    		tdc.createChildObject(rel);
        }
        if ( PreFormTrigger.INFRASTRUCTURE_CONSTANT.equals(childType)) {
    		String rel = "recipes2infrastructure";
    		tdc.createChildObject(rel);
        }
