...
docker rmi tura/hr-manager/wildfly:1.0
...
docker cp\
                $TURA_WORKSPACE_DIRECTORY/etc/wildfly-${wildfly_version}/assets/application-roles.properties\
                $v:/opt/jboss/wildfly/standalone/configuration

docker cp \
                $TURA_WORKSPACE_DIRECTORY/etc/wildfly-${wildfly_version}/assets/application-users.properties\
                $v:/opt/jboss/wildfly/standalone/configuration
...
docker cp \
                $TURA_WORKSPACE_DIRECTORY/etc/wildfly-${wildfly_version}/assets/wf-config.cli\
