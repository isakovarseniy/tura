...
public class GrantedRolePropertyEditor extends PropertyEditor implements
	...
	private void openDialog(Property property) throws Exception {
		...
		array.add("ifGranted");
		array.add("ifAllGranted");
		array.add("ifAnyGranted");
		array.add("ifNotGranted");
	...
	public void paint(Property property, GC gc, int x, int y, int width,
		...
		if (value instanceof GrantedRoleProperty) {
			GrantedRoleProperty link = (GrantedRoleProperty) value;

			if (link != null) {
				String text = link.getGranted();
				if (text != null) {
