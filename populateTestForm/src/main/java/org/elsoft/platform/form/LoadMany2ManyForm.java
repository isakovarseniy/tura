...
public class LoadMany2ManyForm {
	...
	public void execute(RepositoryFactory rf, HashMap<String, Object> context)
		...
		Reader root = new CanvasParser()
				.XWTModelbuilder(
						new File(
								"/home/platform/GITRepo/tura/testform/Many2ManyForm/src/DataDefinition.xwt"),
						 null);
		root.buildAll(context, rf, root, program);

		String frmUUID = Reader.idMAP.get("Many2ManyForm");

		root = new CanvasParser()
				.XWTModelbuilder(
						new File(
								"/home/platform/GITRepo/tura/testform/Many2ManyForm/src/Canvas.xwt"),
						 frmUUID);
		root.buildAll(context, rf, root, program);
		...
		root = new CanvasParser()
				.XWTModelbuilder(
						new File(
								"/home/platform/GITRepo/tura/testform/Many2ManyForm/src/Window.xwt"),
						 frmUUID);
		root.buildAll(context, rf, root, program);
		...
		root = new CanvasParser()
				.XWTModelbuilder(
						new File(
								"/home/platform/GITRepo/tura/testform/Many2ManyForm/src/DetailCanvas.xwt"),
						 frmUUID);
		root.buildAll(context, rf, root, program);

		Iterator<CommandDAO> itr = program.iterator();
		HashMap<String, Object> ctx = new Processor().executor(rf, itr);
		Form frm = (Form) ctx.get("Many2ManyForm");
